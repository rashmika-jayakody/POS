...
class RestaurantCashDrawerController extends Controller
{
    ...
    /**
     * Display the restaurant cash drawer / POS page with products for selection.
     */
    public function index()
    {
        ...
        $productsJson = $products->map(function ($p) {
            ...
            // For restaurants, menu items don't have stock limits (made to order)
            // Set stock to unlimited for restaurant POS
            return [
                'id' => $p->id,
                'name' => $p->name,
                'code' => $p->code ?? '',
                ...
                'stock' => 999999, // Unlimited stock for restaurant menu items
        ...
        // Get tables and waiters for restaurant POS
        $tables = RestaurantTable::where('tenant_id', $user->tenant_id)
        ...
        // Get unpaid orders (orders sent to kitchen but not paid yet)
        $unpaidOrders = RestaurantOrder::where('tenant_id', $user->tenant_id)
            ->whereIn('status', ['confirmed', 'preparing', 'ready', 'served'])
            ->with(['table', 'user', 'items.product'])
            ->orderBy('created_at', 'desc')
            ->get();

        return view('restaurant-cash-drawer.index', compact('categories', 'productsJson', 'invoiceNo', 'storeName', 'currencySymbol', 'taxRate', 'taxLabel', 'inventoryBranchName', 'posShortcuts', 'tables', 'waiters', 'unpaidOrders'));
    }
