...
@section('content')
    ...
    <div class="dashboard-stats animate-in" style="display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; margin-bottom: 24px;">
        <div style="background: #fff; border-radius: 12px; padding: 14px 16px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08); box-sizing: border-box; min-width: 0;">
            ...
            <div style="font-size: 0.72rem; color: #64748B; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; text-transform: uppercase; letter-spacing: 0.3px;">{{ __('Sales this week') }}</div>
        ...
        <div style="background: #fff; border-radius: 12px; padding: 14px 16px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08); box-sizing: border-box; min-width: 0;">
            ...
            <div style="font-size: 0.72rem; color: #64748B; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; text-transform: uppercase; letter-spacing: 0.3px;">{{ __('Users') }}</div>
            ...
            <div style="font-size: 0.7rem; margin-top: 6px; display: flex; align-items: center; gap: 4px; color: #10B981;"><i class="fas fa-user"></i> {{ __('In your store') }}</div>
        </div>
        <div style="background: #fff; border-radius: 12px; padding: 14px 16px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08); box-sizing: border-box; min-width: 0;">
            ...
            <div style="font-size: 0.72rem; color: #64748B; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; text-transform: uppercase; letter-spacing: 0.3px;">{{ __('Units in stock') }}</div>
            ...
            <div style="font-size: 0.7rem; margin-top: 6px;"><a href="{{ route('reports.stock-valuation') }}" style="color: #10B981; text-decoration: none;"><i class="fas fa-chart-line"></i> {{ __('Stock report') }}</a></div>
        </div>
        <div style="background: #fff; border-radius: 12px; padding: 14px 16px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08); box-sizing: border-box; min-width: 0;">
            ...
            <div style="font-size: 0.72rem; color: #64748B; font-weight: 600; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; text-transform: uppercase; letter-spacing: 0.3px;">{{ __('Locations') }}</div>
            ...
            @if(($expiryCount ?? 0) > 0)
                <div style="font-size: 0.7rem; margin-top: 6px;"><a href="{{ route('reports.expiry-tracking') }}" style="color: #EF4444; text-decoration: none;"><i class="fas fa-exclamation-triangle"></i> {{ __(':count expiring soon', ['count' => $expiryCount]) }}</a></div>
            @else
                <div style="font-size: 0.7rem; margin-top: 6px; color: #10B981;"><i class="fas fa-check"></i> {{ __('Branches') }}</div>
            @endif
        </div>
    </div>
    ...
    <div class="dashboard-charts animate-in" style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px;">
        <div style="background: #fff; border-radius: 20px; padding: 24px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08);">
            <h3 style="font-size: 1.05rem; font-weight: 700; color: #0F172A; margin-bottom: 6px; display: flex; align-items: center; gap: 8px;"><i class="fas fa-chart-bar" style="color: #4A9EFF;"></i> {{ __('Sales last 7 days') }}</h3>
            <p style="font-size: 0.8rem; color: #64748B; margin-bottom: 16px;">{{ __('From') }} <a href="{{ route('reports.sales-summary') }}" style="color: #4A9EFF; text-decoration: none;">{{ __('Sales Summary') }}</a></p>
            <div id="wrapSales" style="position: relative; height: 240px; width: 100%;">
                @if(!empty($salesByDay) && count($salesByDay) > 0)
                    <canvas id="chartSales"></canvas>
                @else
                    <div style="height: 200px; display: flex; align-items: center; justify-content: center; background: #F1F5F9; border-radius: 12px; color: #64748B; font-size: 0.9rem;">{{ __('No sales data for the last 7 days') }}</div>
                @endif
            </div>
        </div>
        <div style="background: #fff; border-radius: 20px; padding: 24px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08);">
            <h3 style="font-size: 1.05rem; font-weight: 700; color: #0F172A; margin-bottom: 6px; display: flex; align-items: center; gap: 8px;"><i class="fas fa-chart-pie" style="color: #4A9EFF;"></i> {{ __('Expenses this month') }}</h3>
            <p style="font-size: 0.8rem; color: #64748B; margin-bottom: 16px;">{{ __('From') }} <a href="{{ route('reports.profit-loss') }}" style="color: #4A9EFF; text-decoration: none;">{{ __('Profit & Loss') }}</a></p>
            <div id="wrapExpenses" style="position: relative; height: 240px; width: 100%;">
                ...
                @else
                    <div style="height: 200px; display: flex; align-items: center; justify-content: center; background: #F1F5F9; border-radius: 12px; color: #64748B; font-size: 0.9rem;">{{ __('No expenses this month') }}</div>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="animate-in" style="background: #fff; border-radius: 20px; padding: 24px; border: 1px solid rgba(74, 158, 255, 0.1); box-shadow: 0 2px 8px rgba(10, 26, 61, 0.08); margin-bottom: 24px;">
        <h3 style="font-size: 1.05rem; font-weight: 700; color: #0F172A; margin-bottom: 6px; display: flex; align-items: center; gap: 8px;"><i class="fas fa-boxes-stacked" style="color: #4A9EFF;"></i> {{ __('Stock value by category') }}</h3>
        <p style="font-size: 0.8rem; color: #64748B; margin-bottom: 16px;">{{ __('From') }} <a href="{{ route('reports.stock-valuation') }}" style="color: #4A9EFF; text-decoration: none;">{{ __('Stock Valuation') }}</a></p>
        <div id="wrapStock" style="position: relative; height: 280px; width: 100%;">
            @if(!empty($stockByCategory) && count($stockByCategory) > 0)
                <canvas id="chartStock"></canvas>
            @else
                <div style="height: 200px; display: flex; align-items: center; justify-content: center; background: #F1F5F9; border-radius: 12px; color: #64748B; font-size: 0.9rem;">{{ __('No stock data') }}</div>
            @endif
        </div>
    </div>
    ...
    <div class="section animate-in">
        <h2 class="section-title"><i class="fas fa-bolt"></i> {{ __('Quick Actions') }}</h2>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px;">
            @php
                $posType = auth()->user()->tenant?->pos_type ?? 'retail';
            @endphp
            @if($posType === 'restaurant')
            <a href="{{ route('restaurant-cash-drawer.index') }}" class="btn btn-primary" style="justify-content: center;"><i class="fas fa-utensils"></i> {{ __('Restaurant POS') }}</a>
            @else
            <a href="{{ route('cash-drawer.index') }}" class="btn btn-primary" style="justify-content: center;"><i class="fas fa-cash-register"></i> {{ __('Cash Drawer') }}</a>
            @endif
            <a href="{{ route('reports.index') }}" class="btn btn-primary" style="justify-content: center;"><i class="fas fa-chart-line"></i> {{ __('Reports') }}</a>
            <a href="{{ route('users.create') }}" class="btn btn-secondary" style="justify-content: center;"><i class="fas fa-user-plus"></i> {{ __('Add User') }}</a>
            <a href="{{ route('products.index') }}" class="btn btn-secondary" style="justify-content: center;"><i class="fas fa-box-open"></i> {{ __('Products') }}</a>
            <a href="{{ route('company-other-expenses.index') }}" class="btn btn-secondary" style="justify-content: center;"><i class="fas fa-receipt"></i> {{ __('Expenses') }}</a>
        </div>
    </div>
@endsection
