...
<form method="get" action="{{ route($routeName) }}" class="report-filter-form" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: flex-end; margin-bottom: 20px;">
    @if($showDate)
        <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; color: var(--gray-500); margin-bottom: 4px;">From</label>
        ...
        <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; color: var(--gray-500); margin-bottom: 4px;">To</label>
    ...
    @if($showBranch)
        <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; color: var(--gray-500); margin-bottom: 4px;">Branch</label>
    ...
    @if($showPeriod)
        <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; color: var(--gray-500); margin-bottom: 4px;">Group by</label>
    ...
    @if($showType && isset($type))
        <div>
            <label style="display: block; font-size: 0.75rem; font-weight: 600; color: var(--gray-500); margin-bottom: 4px;">Show</label>
            <select name="type" class="filter-input" style="padding: 8px 12px; border: 1px solid var(--gray-300); border-radius: 8px; font-size: 0.9rem;">
                ...
                <option value="soon" {{ ($type ?? '') === 'soon' ? 'selected' : '' }}>Expiring soon (30 days)</option>
            </select>
        </div>
    @endif
